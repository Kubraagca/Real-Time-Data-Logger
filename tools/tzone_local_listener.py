...
def decode_temperature(raw_value: Optional[int]) -> Optional[float]:
    if raw_value is None or raw_value == 0x8000:
        return None
    sensor_abnormal = (raw_value & 0x8000) != 0
    if sensor_abnormal:
        return None
...
class TzoneTcpHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self) -> None:
        ...
        while True:
            ...
            print("[PARSED]", json.dumps(asdict(parsed), ensure_ascii=False))
            ...
            if parsed.protocol == "tzone-binary":
                ...
                print("[STATUS]", json.dumps(summary, ensure_ascii=False))
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Tzone TT18 local TCP listener. Cihaz baglaninca raw ve parse edilen veriyi consola basar."
    )
    parser.add_argument("--host", default="0.0.0.0")
    parser.add_argument("--port", type=int, default=18801)
    parser.add_argument("--ack", action="store_true", help="MessageID okunursa @ACK don")
    args = parser.parse_args()

    TzoneTcpHandler.ack_enabled = args.ack

    server = ThreadedTCPServer((args.host, args.port), TzoneTcpHandler)
    ...
    print(f"[SERVER] listening on {args.host}:{args.port} ack={args.ack}")
    print("[SERVER] Gercek cihaz paketi icin rawHex baslangici 545A olmali.")
    print("[SERVER] 160301 ile baslayan veri TLS'tir, cihaz sicaklik paketi degildir.")
